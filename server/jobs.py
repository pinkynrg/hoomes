...
def fetch_homes(comune): 
  # Check if the lowest updated_at of House is > 1 day ago
    one_day_ago = datetime.now() - timedelta(days=1)
    lowest_updated_at = House.select(fn.Min(House.updated_at)).where(House.comune == comune).scalar()

    if lowest_updated_at is None or lowest_updated_at < one_day_ago:
        # Fetch data from Idealista
        idealista_data = Idealista.fetch(comune)
    ...
    records = House.select().where(House.comune == comune)
